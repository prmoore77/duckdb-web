...
| `struct_insert(`*`struct`*`, name := any`*`, ...)` | Add new named entry into an existing `STRUCT`. The entry name will be the bound variable name.                                                                                                                             | `struct_insert({'a': 1}, b := 2)`    | `{'a': 1, 'b': 2}`           |
